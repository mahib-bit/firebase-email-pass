...
const Profile = () => {
  ...
  // Update Profile using Promise Chaining
  ...
  // Method 1: Send Password Reset Email Link
  const handleResetPassword = () => {
    if (user?.email) {
      sendPasswordResetEmail(auth, user.email)
        .then(() => toast.success('Password reset link sent! Check your email inbox.'))
  ...
  // Method 2: Update Password Directly In-App using Promise Chaining
  const handleDirectPasswordUpdate = (e) => {
    e.preventDefault();
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6">
            ...
            <div className="space-y-1">
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
                ...
                <span className={`px-3 py-0.5 rounded-full text-xs font-semibold border backdrop-blur-md flex items-center gap-1 ${
                  user?.emailVerified 
            ...
            <div className="p-6 sm:p-8 rounded-3xl bg-white/10 backdrop-blur-xl border border-white/20 shadow-[0_8px_32px_0_rgba(0,0,0,0.37)] space-y-4 flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs text-slate-300 leading-relaxed mb-4">
                  Manage your credentials via email link or direct update.
                </p>
                ...
                <div className="space-y-3">
                  ...
                  >
                    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700 ease-out" />
                    <span className="relative z-10 flex items-center gap-2">
                      <Send className="w-4 h-4 text-indigo-300" /> Send Password Reset Email
                    </span>
                    <Send className="relative z-10 w-4 h-4 text-indigo-300 group-hover:text-white" />
                  </button>
                  ...
                  >
                    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700 ease-out" />
                    <span className="relative z-10 flex items-center gap-2">
                      <Key className="w-4 h-4 text-purple-300" /> {showPasswordForm ? 'Close In-App Form' : 'Change Password In-App'}
                    </span>
                    <Key className="relative z-10 w-4 h-4 text-purple-300 group-hover:text-white" />
                  </button>

                  {showPasswordForm && (
                    <form onSubmit={handleDirectPasswordUpdate} className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-3 pt-4">
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            required
                            className="w-full pl-9 pr-3 py-2 rounded-xl bg-white/5 border border-white/15 text-xs text-white placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-indigo-400"
                          />
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        className="w-full py-2.5 rounded-xl text-xs font-semibold bg-indigo-600 hover:bg-indigo-500 text-white transition-all shadow active:scale-95 disabled:opacity-50"
                      >
                        {passwordLoading ? 'Updating Password...' : 'Update Password Now'}
                      </button>
                    </form>
                  )}
                </div>
